...
public class doorEarthAngleRoom : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.tag == "Player")
        {
            ...
            if ((player.Element != ElementType.Water && player.Stones[(int)ElementType.Water] > 0) || (player.Element == ElementType.Water && player.Stones[(int)ElementType.Water] > 1))
            {
